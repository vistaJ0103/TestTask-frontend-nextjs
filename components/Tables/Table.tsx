...
const Table = () => {
  ...
  const deleteUser = async (id: string) => {
    ...
    if (res) {
      toastNotification("User Deleted Success", "success", 3000);
      const updatedUserList = subAllUsers.filter((user: IUser) => user._id !== id);
      setSubAllUsers(updatedUserList);
      return res.msg;
    } else {
      toastNotification("User Deleted Failed", "error", 3000);
    }
  }

  const userSetting = async (id: string) => {
    const selectedUser = subAllUsers.find((user: IUser) => user._id === id);
    if (selectedUser) {
      router.push({
        pathname: "/settings",
              ...
              <tr key={key}>
                ...
                <td className="border-b  py-5 px-4 border-strokedark">
                  <div className="flex items-center space-x-3.5">
                    <button className="hover:text-primary" onClick={() => userSetting(subAllUser._id)}>
                    ...
                    <button className="hover:text-primary" onClick={() => deleteUser(subAllUser._id)}>
                      <svg
                        className="fill-current"
                        width="18"
                        height="18"
